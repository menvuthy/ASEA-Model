...
for i in range(grid_num.getInfo()):
  ...
  image = download.image_no_clouds(ls_feature[i], start_date, end_date)
...
output = os.path.join('/content/temp/main/image_snrgb_'+date[0][:4]+'.tif')
with rasterio.open(output, "w", **out_meta) as dest:
    dest.write(mosaic.astype(np.float32))

# Clip image to aoi geometry
img_grid = rasterio.open('/content/temp/main/image_snrgb_'+date[0][:4]+'.tif')
...
output = os.path.join('output/landsat-image/30m-images/image_snrgb_'+date[0][:4]+'.tif')
with rasterio.open(output, "w", **out_meta) as dest:
